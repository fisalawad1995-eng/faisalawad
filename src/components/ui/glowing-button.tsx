...
export function GlowingButton({
  ...
  const glowColorVia = hexToRgba(glowColor, 0.035); // خفضنا الشفافية للنصف تقريباً
  const glowColorTo = hexToRgba(glowColor, 0.1);    // خفضنا الشفافية من 0.2 إلى 0.1
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "before:absolute before:w-[5px] hover:before:translate-x-full before:transition-all before:duration-200 before:h-[60%] before:bg-[var(--glow-color)] before:right-0 before:rounded-l before:shadow-[-2px_0_5px_var(--glow-color)] z-10",
        className
      )}
    >
      {children}
